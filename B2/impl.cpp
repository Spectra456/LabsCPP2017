...
TextEditer::TextEditer(const size_t bufferSize, std::istream *input):
  buffer_(),
  input_(input),
  lastChar_(' '),
...
void TextEditer::readNumber(std::string& tempString) {
  size_t counter = 1;
...
void TextEditer::readWord(std::string& tempString) {
  size_t counter = 1;
  while (input_->get(lastChar_)) {
...
void TextEditer::readDash(std::string &tempString) {
  std::string dash(1, '-');
  dash.reserve(4);
  if (tempString.back() == '-') {
    dash.push_back('-');
